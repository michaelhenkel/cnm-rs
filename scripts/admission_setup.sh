...
kubectl delete mutatingwebhookconfiguration cnm-admission-controller || true
# If behind a service:
...
openssl req -nodes -new -x509 \
    -keyout ca.key \
    -out ca.crt -subj "/CN=cnm-admission-controller"
...
openssl req -new -key admission-controller-tls.key \
    -subj "/CN=cnm-admission-controller" \
...
# Sanity:
kubectl get mutatingwebhookconfiguration cnm-admission-controller -oyaml